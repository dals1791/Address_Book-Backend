...
const connections = {
  Mutation: {
    ...
    addConnectionToGroup: async (parent, args, context, info) => {
      ...
      try {
        ...
        if (connectedStatus === false) {
          throw new Error(
            "You are not connected, connect with this user first"
          );
        }
    ...
    destroyConnectionFromGroup: async (parent, args, context, info) => {
      const { groupId } = args;
      const { handle } = args;
      const { userId } = context;
      if (!context || userId == null || userId == "") {
